...
### Added
- **Android — native non-UI component hosts + real JNI (Phase 5)**: Grows the Android target from "a WebView in an Activity" into a first-class mobile shell, on the critical path for neurophone#83 (migrating a banned-Kotlin Android app onto gossamer).
  - `src/interface/ffi/src/jni.zig` (new) — a REAL JNI binding in pure Zig: models the `JNINativeInterface`/`JNIInvokeInterface` tables (ordinal-indexed), uses the `...A`-variant calls (no C varargs across the boundary), caches the `JavaVM` for thread attach, and uses global refs. The previous bridge declared `extern fn jni_*` symbols that were defined nowhere, so it could never link.
  - Foreground **`Service`** (`android_service.zig`), **`BroadcastReceiver`** (`android_receiver.zig`) and **`AppWidgetProvider`** (`android_widget.zig`) hosts, on a process-global handler registry (`android_components.zig`). Apps register pure handlers via `gossamer_service_bind` / `gossamer_receiver_bind` / `gossamer_widget_bind` — the same C-ABI callback shape as webview IPC — and never touch JNI. Inbound events are flat JSON; outbound directives are `key\tvalue` records the generated Java applies (foreground notification, RemoteViews render, start-service).
  - `src/interface/abi/AndroidComponents.idr` (new, wired into `gossamer-abi.ipkg`) — terminal-teardown and dispatch-only-while-live proofs per component; the long-lived Service is a `LinearHandle` over the same Allocated/Active/Consumed machine as the webview, so linearity crosses the JNI boundary. Zero `believe_me`.
  - `webview_android.zig` rewritten onto `jni.zig`; fixes latent bugs (storing local refs that die after `nativeInit`; double JS-interface registration). `main.zig` now selects the Android backend by `abi == .android` (it was falling through to GTK, leaving `webview_android.zig` dead). `build.zig` links NDK `log`/`android` and skips WebKitGTK for the android abi.
  - `tools/android-codegen/` (new) generates the JVM-bytecode shims into `android/gossamer-generated/` (Activity, Bridge, Service, Receiver, Widget + manifest skeleton). The hand-authored `android/src` Java is **deleted** — the only Java in the repo is now generated. Generated Java compiles clean against Android API stubs; every `native` method has a matching `export fn Java_*` (symbol parity checked).
  - Host-runnable component tests wired into `zig build test` (`test-android`); `just android-gen` / `just android-build` recipes.
  - Design + "ready-to-depend-on" checklist: `docs/architecture/android-components.adoc`.
