...
* `src/Gossamer.affine` — *type-checks clean* against the AffineScript compiler
  ...
  binding.
* `@gossamer/api` (MPL-2.0) publishes native AffineScript Bun-ESM output:
  `bun run build` runs `affinescript compile --bun-esm` to (re)generate
  `src/Gossamer.bun.js`, the package's `exports` entry (built at publish,
...
(IPC is `IO`; the host round-trip is `Async`). The Bun-ESM host profile
