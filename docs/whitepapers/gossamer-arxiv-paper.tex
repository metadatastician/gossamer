...
\label{thm:no-escape}
If $e$ is a well-typed Ephapax expression in linear mode with
$\texttt{let!}~x = v~@r~\texttt{in}~e'$, then $x$ does not appear
free in any continuation outside the scope of region $r$.
\end{theorem}

\begin{proof}
By the linear typing rule for \texttt{let!}, the variable $x$ must
appear exactly once in $e'$. Since $e'$ is the body of the region
block, and the only continuation after the region block does not
include $x$ in its context (the linear context is split, not shared),
$x$ cannot escape. \qed
\end{proof}
...
and \texttt{gtk\_main} for the event loop.
...
fn main() -> i32 {
    region app:
        ...
        let! webview = Shell.loadHTML(webview, "<html>...")
        Shell.run(webview)
...
Removing \texttt{Bridge.close(channel)} produces a compile-time error:
the linear variable \texttt{channel} is unconsumed at region exit.
Using \texttt{webview} after \texttt{Shell.run} produces a type error:
