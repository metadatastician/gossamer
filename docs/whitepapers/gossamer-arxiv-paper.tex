...
architecture using Idris2 ABI definitions and Zig FFI bindings,
...
and evaluate binary size (1.2\,MB vs.\ Electron's 220\,MB and
Tauri's 3\,MB), memory safety guarantees, and startup characteristics.
...
We evaluate Gossamer along three axes: binary size, memory safety
guarantees, and startup characteristics. Since Gossamer targets
the same use case as Electron and Tauri (desktop applications with
web frontends), we compare directly with these two frameworks.

\subsection{Binary Size}

Table~\ref{tab:binary} reports the size of a minimal ``Hello World''
application (a single window displaying \texttt{<h1>Hello</h1>})
built with each framework.

\begin{table}[ht]
\centering
\caption{Binary size comparison (Hello World application)}
\label{tab:binary}
\small
\begin{tabular}{@{}lrrl@{}}
\toprule
\textbf{Framework} & \textbf{Binary} & \textbf{Runtime Deps} & \textbf{Total} \\
\midrule
Electron 33      & $\sim$2\,MB    & Chromium + Node.js & $\sim$220\,MB \\
Tauri 2.0        & $\sim$3\,MB    & OS webview         & $\sim$3\,MB \\
Wails 2.9        & $\sim$7\,MB    & OS webview         & $\sim$7\,MB \\
Neutralinojs 5   & $\sim$2\,MB    & OS webview         & $\sim$2\,MB \\
\textbf{Gossamer 0.1} & $\sim$1.2\,MB & OS webview     & $\sim$1.2\,MB \\
\bottomrule
\end{tabular}
\par\smallskip
{\footnotesize Electron total includes the bundled Chromium engine.
Gossamer and Tauri use the OS-provided webview (WebKitGTK on Linux).
Gossamer measured with \texttt{zig build -Doptimize=ReleaseSafe}
producing \texttt{libgossamer.so}.}
\end{table}

Gossamer's size advantage over Tauri stems from two factors:
(1)~the Zig FFI layer produces a single shared library with no
runtime dependencies beyond libc and the platform webview, and
(2)~the Ephapax application layer compiles to compact native code
with no garbage collector runtime, no serialisation framework, and
no async runtime.

\subsection{Memory Safety Guarantees}

Table~\ref{tab:guarantees} compares the static (compile-time) and
dynamic (runtime) safety guarantees of each framework.
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Property} & \textbf{Electron} & \textbf{Tauri} & \textbf{Gossamer} \\
\midrule
No use-after-free     & Runtime (GC)   & Runtime (Rc)   & \textbf{Compile-time} \\
No double-free        & Runtime (GC)   & Runtime (Rc)   & \textbf{Compile-time} \\
No resource leak      & No guarantee   & No guarantee   & \textbf{Compile-time} \\
IPC type agreement    & None           & Codegen        & \textbf{Compile-time} \\
Permission soundness  & Opt-in sandbox & Runtime JSON   & \textbf{Compile-time} \\
No region escape      & N/A            & Borrow checker & \textbf{Proven} \\
\bottomrule
\end{tabular}
\par\smallskip
{\footnotesize ``Compile-time'' means the property is enforced by the
type checker: violating programs are rejected before execution.
``Proven'' means a formal proof exists in the Idris2 ABI layer.
``Runtime'' means the property is enforced dynamically.}
\end{table}

Gossamer is the only framework where \emph{all six properties} are
enforced statically. Tauri's borrow checker prevents some use-after-free
in Rust backend code, but IPC type agreement is ensured only by code
generation (not verified by the Rust compiler), and permissions are
runtime configuration. Electron provides none of these guarantees at
compile time.

\subsection{Startup Time Characteristics}

Gossamer's startup path is minimal:
\begin{enumerate}[nosep]
  \item \texttt{dlopen("libgossamer.so")} --- load the FFI library
  \item \texttt{gossamer\_create()} --- initialise GTK and create a
    WebKitGTK webview (platform-level cold start)
  \item \texttt{gossamer\_load\_html()} --- inject HTML content
  \item \texttt{gossamer\_run()} --- enter the GTK event loop
\end{enumerate}

Steps 1, 3, and 4 are negligible ($<$1\,ms each). Step~2 is dominated
by the platform webview initialisation (GTK \texttt{gtk\_init} and
WebKitGTK process creation), which is shared with Tauri and other
OS-webview frameworks. Gossamer adds no overhead beyond the platform
cost: there is no GC to initialise, no JavaScript engine to warm up
(beyond the webview's own), no async runtime to start, and no
configuration file to parse.

By contrast, Electron must start a full Chromium browser process and
a Node.js runtime, adding 200--500\,ms to the platform baseline.
Tauri starts a Tokio async runtime and parses its JSON configuration,
adding approximately 10--30\,ms.

\subsection{Threats to Validity}

The primary limitation of this evaluation is the absence of
large-scale benchmarks on real-world applications. The binary size
and safety guarantee comparisons are objective, but startup time
analysis is qualitative rather than measured. We plan to conduct
formal benchmarks as future work (\S\ref{sec:future}).

Additionally, Gossamer currently supports only Linux (WebKitGTK).
Cross-platform performance may differ on macOS (WKWebView) and
Windows (WebView2), where the OS webview has different cold-start
characteristics.
