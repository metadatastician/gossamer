...
Script locations (repo-relative; `repo:path` = that repo's checkout root):
- `run-maintenance.sh` at the estate repo root
...
```bash
# PANIC_ATTACKER = your panic-attacker checkout (repo:path → panic-attacker:target/release/panic-attack)
PANIC_BIN="$PANIC_ATTACKER/target/release/panic-attack"
...
```bash
cd "$GITBOT_FLEET"   # your gitbot-fleet checkout
just enroll-repos
```
...
```bash
cd "$GITBOT_FLEET"   # your gitbot-fleet checkout
just enroll-repos "$ESTATE_ROOT" true
```
