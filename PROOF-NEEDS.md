...
- `CapabilityAuthenticity.idr` — Capability authenticity proofs — **relocated to the groove repo's proof package** (PROOFS-1, 2026-07-27)
- `GrooveTermination.idr` — Groove handshake termination proof ✅ — **relocated to the groove repo's proof package** (PROOFS-1, 2026-07-27)
...
- `src/interface/abi/TransmuteStateMachine.idr` — Transmute mode-switch state machine ✅ NEW 2026-07-09
- `IPCDispatch.idr` — IPC handler type safety, 25 handlers (GS2) ✅ — **relocated to the groove repo's proof package** (PROOFS-1, ruled 2026-07-27; no longer in this repo)
- **All 15 in-repo ABI modules build green and pass `idris2 --typecheck` cleanly**, split across two de-conflated packages (`gossamer#95`): the groove-agnostic **shell** `gossamer-abi.ipkg` (12 modules, incl. the generated `ForeignGen`) and the **groove** `gossamer-groove.ipkg` (3: `Groove`, `GrooveLinearity`, `GrooveResidue`), which *depends on* the shell. Three further modules — `IPCDispatch`, `CapabilityAuthenticity`, `GrooveTermination` — were **relocated to the groove repo's proof package** (PROOFS-1, ruled 2026-07-27) and are gated there, not here. The dependency points one way only — no shell module imports a groove module. The canonical sources live in `src/interface/abi/`; `src/interface/Gossamer/ABI/<M>.idr` are symlinks (a single source of truth, no drift). `scripts/check-abi-decoupling.sh` gates both invariants in CI. Prior to `gossamer#22` / `#36` / `#40` / `#41`, several modules were excluded from the ipkg and had never been built — their PROOF-NEEDS ✅ markers reflected an unverified posture; build is now the oracle.
...
- ~~**Groove protocol handshake**~~: ✅ Proved in GrooveTermination.idr — terminates in ≤4 steps, no privilege escalation, deterministic. **Proof relocated to the groove repo's package (PROOFS-1, 2026-07-27)** — statement unchanged, gated there.
...
- ~~**IPC handler type safety (GS2)**~~: ✅ Proved in IPCDispatch.idr — all 25 handlers have declared input/output types, dispatch is total, capability-guarded vs plain commands classified, distinctness witnesses (2026-04-11). **Proof relocated to the groove repo's package (PROOFS-1, 2026-07-27)** — statement unchanged, gated there.
- ~~**Transmute mode-switch correctness**~~: ✅ Proved in `TransmuteStateMachine.idr` (2026-07-09) — the "killer feature" transition relation (19 of 36 ordered pairs legal, 14 constructors) with: `validSound`/`validComplete` (the Bool decision function `validTransmute` IS the relation — and the Zig runtime's `validTransition` in `main.zig` mirrors it arm-for-arm, pinned by a 36-pair matrix test); `everyModeHasExit` (no absorbing state — dual of `closedIsTerminal`); `guiAlwaysRecoverable` (gui reachable from every mode, ≤2 steps); `attachExitsOnlyDetach` (PanLL slot acquire/release bracket); **`guiHasNoBackup`** (the backup-bit machine: restore-on-every-gui-entry provably leaves no live JS backup in gui) and **`b1BugWitness`** (the old restore-only-from-terminal-modes guard provably reached gui-with-live-backup via `tui→panll_detach→gui` — the stuck-render bug, stated as a theorem); `toFromId`/`toIntInjective` (C-ABI encoding round-trip). The runtime now REJECTS illegal transitions (`invalid_param` + named message) instead of blindly switching, and eval/attach failures no longer desync the stored mode. Zero `believe_me`.
